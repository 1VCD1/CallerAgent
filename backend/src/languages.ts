...
export interface LangConfig {
  ...
  humanBridgeMessage: (userName?: string) => string; // Spoken to representative when human detected
...
- 🚫 press_key("0") ONLY helps when an operator/representative transfer is on offer. On a HOLD/queue message ("please hold", "all agents busy", "remain on the line") → wait, do NOT press 0. On a conversational assistant asking what you need ("tell me how I can help", "please let me know how I can help") → say_phrase your need. Pressing 0 there does NOTHING and loops you back — never mash 0 against the same response.
...
- "Please leave a voicemail" / "Leave a message after the beep" (leaving a MESSAGE only)
- "There are no agents available at this time"
- "Goodbye" / call disconnected by IVR

⚠️ NOT voicemail: "provide / say / enter your phone NUMBER after the tone" is a CALLBACK NUMBER request, even though it mentions a tone. say_phrase the user's phone number (see CALLBACK RULE) — do NOT end_call as voicemail.
...
- 🚫 press_key("0") 只有在「有提供轉接真人」時才有用。聽到等候訊息（「請稍候」「專員忙線中」「請勿掛斷」）→ 用 wait，不要按 0。對話式語音助理問你需要什麼（「請告訴我能為您做什麼」）→ say_phrase 說出你的需求。對這些情況按 0 完全沒用、只會鬼打牆——絕不要對同一句重複按 0。
...
- 「請在嗶聲後留言」/ 語音信箱（只有「留下訊息」才算）
- 「目前無客服人員」
- 「感謝您的來電，再見」/ IVR 主動掛斷

⚠️ 不是語音信箱：「請在嗶聲後提供／說出／輸入您的電話號碼」是要回撥號碼（即使提到嗶聲也一樣）→ 用 say_phrase 報出使用者電話號碼（見 CALLBACK 規則），不要當成 voicemail 而 end_call。
...
- 🚫 press_key("0") 只有在「有提供转接人工」时才有用。听到等候讯息（「请稍候」「专员忙线中」「请勿挂断」）→ 用 wait，不要按 0。对话式语音助理问你需要什么（「请告诉我能为您做什么」）→ say_phrase 说出你的需求。对这些情况按 0 完全没用、只会鬼打墙——绝不要对同一句重复按 0。
...
- 「请在提示音后留言」/ 语音信箱（只有「留下消息」才算）
- 「目前无客服人员」
- 「感谢您的来电，再见」/ IVR 主动挂断

⚠️ 不是语音信箱：「请在提示音后提供／说出／输入您的电话号码」是要回拨号码（即使提到提示音也一样）→ 用 say_phrase 报出用户电话号码（见 CALLBACK 规则），不要当成 voicemail 而 end_call。
...
export const LANGUAGES: Record<Lang, LangConfig> = {
  'en': {
    ...
    humanBridgeMessage: (name) => name
      ? `Hi! Thanks for taking the call — one moment, connecting you with ${name} right now.`
      : 'Hi! Thanks for taking the call — one moment, I\'m connecting you right now.',
